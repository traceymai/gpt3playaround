...
def transform_txt(filename, num_training_per_cate): # n is the number of randomly chosen input instances wanted for each sentiment category
    ...
    """
    df_train_all = pd.read_csv(filename, skiprows=1, header = 0, encoding = "utf8", sep = ":->", nrows = 28)
    ...
    all_data = pd.read_csv(filename, skiprows = 31, header = 0, encoding = "utf8", sep = ":->", nrows = 30)
...
def write_output(engine, temp, max_tokens, all_data, df_used):
    ...
    mixed_df.to_csv("outf{}.txt".format(temp), sep = " ", mode = "a")
